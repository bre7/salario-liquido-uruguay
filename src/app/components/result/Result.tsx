...
import { ADICIONAL_FONDO_SOLIDARIDAD, BPC, IRPF_FRANJAS, NUMERAL_CURRENCY_FORMAT_STRING, NUMERAL_FORMAT_STRING } from "#data/constants"
import "./Result.scss"
  ...
  return (
    <div className="result">
      ...
      <div className="liquido">
        ...
        <span className="liquido-dato">{numeral(salarioLiquido).format(NUMERAL_CURRENCY_FORMAT_STRING)}</span>
      ...
      <div className="detalleBPS">
        ...
        <span className="tablaBPS-dato">{numeral(aportesJubilatorios).format(NUMERAL_FORMAT_STRING)}</span>
        <span className="tablaBPS-label">FONASA</span>
        <span className="tablaBPS-dato">{numeral(aportesFONASA).format(NUMERAL_FORMAT_STRING)}</span>
        ...
        <span className="tablaBPS-dato totalBPS">{numeral(totalBPSRedondeado()).format(NUMERAL_FORMAT_STRING)}</span>
      </div>
          ...
          IRPF_FRANJAS.map((franja, index) => {
            return (
              <React.Fragment key={`irpf-${index}`}>
                ...
                <span className="tablaIRPF-dato">{numeral(detalleIRPF?.impuestoFranja[index] ?? 0).format(NUMERAL_FORMAT_STRING)}</span>
              </React.Fragment>
            )
          })
        }
      ...
      <div className="resumenIRPF">
        ...
        <span className="resumenIRPF-dato resumenIRPF-deducciones">{detalleIRPF?.deducciones ? numeral(detalleIRPF.deducciones).format(NUMERAL_CURRENCY_FORMAT_STRING) : 0}</span>
        <span className="resumenIRPF-label">Tasa deducciones:</span>
        <span className="resumenIRPF-dato resumenIRPF-tasa">{detalleIRPF?.tasaDeducciones ?? 0}%</span>
        <span className="resumenIRPF-labelTotal">Total IRPF:</span>
        <span className="resumenIRPF-dato resumenIRPF-total">{numeral(totalIRPF).format(NUMERAL_CURRENCY_FORMAT_STRING)}</span>
      </div>
      ...
      <div className="detalleProfesionales">
        ...
        <span className="tablaProfesionales-dato">{numeral(totalFondoSolidaridadRedondeado()).format(NUMERAL_CURRENCY_FORMAT_STRING)}</span>
        <span className="tablaProfesionales-label">Aportes CJPPU / Caja Notarial:</span>
        <span className="tablaProfesionales-dato">{numeral(aportesCJPPURedondeado()).format(NUMERAL_CURRENCY_FORMAT_STRING)}</span>
      </div>
    </div>
  )
}
